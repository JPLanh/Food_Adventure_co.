...
//	ClientLoyaltyUI(){
		currentUser = getUser;
		ArrayList<Reward> rewardList = HttpRequests.getAllRewards();
		...
		for (int x = 0; x < rewardList.size(); x++) {
			frameComponents.add(new PurchaseItem(rewardList.get(x).getName(), 250 + (160*x), 150, 150, 150, rewardList.get(x).getCoin() + " coins", "Purchase " + rewardList.get(x).getName()));
		}
	}
	...
	public String clickAction(int mouseX, int mouseY) {
		...
		if (getAction != null) {
			...
			if (splitAction[0].equals("Purchase")) {
				HttpRequests.purchaseReward(currentUser, getAction.substring(9));
				HttpRequests.finishPurchase(currentUser, getAction.substring(9));
